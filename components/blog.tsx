...
interface BlogItem {
  ...
  title: string;
  excerpt: string;
  category: "tips" | "stories" | "promotions";
  date: string;
  views?: number;
}
  ...
  useEffect(() => {
    if (inView && hasMore) {
      setTimeout(() => {
        setVisibleBlogs((prev) => {
          const newValue = prev + 3;
          if (newValue >= blogs.length) setHasMore(false);
          return newValue;
        });
      }, 500);
    }
  }, [inView, hasMore, blogs.length]);
        ...
        <div className="flex flex-wrap gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => setActiveFilter("tips")}>
                {t("blog.gameTips")}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setActiveFilter("stories")}>
                {t("blog.userStories")}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setActiveFilter("promotions")}>
                {t("blog.promotions")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="gap-1">
                <Calendar className="w-4 h-4" />
                <span>
                  {sortBy === "latest" ? "Latest First" : "Most Popular"}
                </span>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => setSortBy("latest")}>
                <Clock className="w-4 h-4 mr-2" /> Latest First
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortBy("popular")}>
                  ...
                  >
                    <Link href={`/blog/${blog._id}`} className="block h-full">
                      <Card className="overflow-hidden h-full flex flex-col hover:shadow-lg transition-all duration-300 border-2 border-transparent hover:border-primary/20">
                        ...
                        >
                          <img
                            src={`/abstract-geometric-shapes.png?height=${
                              isFeatured ? 400 : 200
                            }&width=${isFeatured ? 800 : 400}&query=${
                              blog.title
                            }`}
                            alt={blog.title}
                            className="w-full h-full object-cover transition-transform hover:scale-105 duration-700"
                        ...
                        <CardHeader
                          className={`pb-2 ${isFeatured ? "pt-5" : "pt-4"}`}
                        >
                          <CardTitle
